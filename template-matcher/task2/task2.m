...
% get template imgs
templateImgs = createRotatedImages(trainingDir, 'png', numLvls, sigma, octave, numRots, skipNum);
...
for i = 1 %: length(testImgs)
    ...
    matches = zeros(length(templateImgs)*15,9); % [row, col, height, width, intensity, id, rot, scaleR, scaleC]
    ...
    for templateInd =  1 : length(templateImgs) %[11,16,22,24,38,44] %
        ...
        templateMatches = zeros(numLvls*numRots,9); %assumes 1 appearance of each template
        ...
        templateMatchesSupp = nonMaxSupp(templateMatches, IoU_thresh, false);
    ...
    boxes = nonMaxSupp(weightedMatches, IoU_thresh, false);
